...
def test_parse_config_returns_instance_of_config(all_generated_configs):
    """test that ``vak.config.parse.from_toml`` returns an instance of ``vak.config.parse.Config``"""
    for toml_path in all_generated_configs:
        config_obj = vak.config.parse.from_toml(toml_path)
        assert isinstance(config_obj, vak.config.parse.Config)


def test_invalid_section_raises(invalid_section_config_path):
    """test that a .toml config with an invalid section raises a ValueError"""
    with pytest.raises(ValueError):
        vak.config.parse.from_toml(invalid_section_config_path)


def test_invalid_option_raises(invalid_option_config_path):
    """test that a .toml config with a section containing an invalid option raises a ValueError"""
    with pytest.raises(ValueError):
        vak.config.parse.from_toml(invalid_option_config_path)
...
def test_train_and_learncurve_defined_raises(invalid_train_and_learncurve_config_toml):
    ...
    with pytest.raises(ValueError):
        vak.config.parse.from_toml(invalid_train_and_learncurve_config_toml)
